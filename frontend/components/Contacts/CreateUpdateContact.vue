  ...
  methods: {
    displayField(field) {
      ...
      if (field.type == "file") {
        ...
        const readableDate = field.value.replace("T11:11:00Z", "")
        return `
          <p>${field.label} ${requiredTitle}</p>
          <input type="date" name="${
            field.name
          }" value="${readableDate}" ${required}>
        `
        ...
        return `
          <p>${field.label} ${requiredTitle}</p>
          <textarea name="${field.name}" rows="8" placeholder="${
          field.placeholder
    ...
    getFormValues(submitEvent) {
      ...
      const date_of_birth =
        submitEvent.target.elements.date_of_birth.value + "T11:11:00Z"
      const avatar =
        submitEvent.target.elements.avatar.value ||
        "http://pluspng.com/img-png/user-png-icon-file-user-icon-black-01-png-311.png"
      const notes = submitEvent.target.elements.notes.value || null
      if (this.action == "create") {
    ...
    async createProfile(first_name, last_name, date_of_birth, avatar, notes) {
      await this.$axios
        ...
        .catch(error => {
          this.error = `Problem submitting New Contact. Try again later. ${error}`
        })
    },
    async updateProfile(first_name, last_name, date_of_birth, avatar, notes) {
      await this.$axios
        .put(`contacts/${this.edit}/`, {
          first_name: first_name,
          last_name: last_name,
          date_of_birth: date_of_birth,
          avatar: avatar,
        ...
        .catch(error => {
          this.error = `Problem Editing the contact. Try again later. ${error}`
        })
    }
  }
}
