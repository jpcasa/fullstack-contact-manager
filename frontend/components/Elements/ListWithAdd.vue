...
export default {
  ...
  methods: {
    ...
    async createAddress(title, id) {
      await this.$axios.post('addresses/', {
        title: title,
        owner: 1,
        contact_id: id
      })
    ...
    async deleteAddress(id) {
      await this.$axios.delete(`addresses/${id}/`)
      .then(response => {
    ...
    async createPhone(number, id) {
      await this.$axios.post('phone-numbers/', {
        number: number,
        owner: 1,
        contact_id: id
      })
    ...
    async deletePhone(id) {
      await this.$axios.delete(`phone-numbers/${id}/`)
      .then(response => {
        if (response.status == '204') {
          this.listItems = this.listItems.filter(item => item.id !== id)
        }
    ...
    async deleteEmail(id) {
      await this.$axios.delete(`emails/${id}/`)
      .then(response => {
        if (response.status == '204') {
          this.listItems = this.listItems.filter(item => item.id !== id)
        }
